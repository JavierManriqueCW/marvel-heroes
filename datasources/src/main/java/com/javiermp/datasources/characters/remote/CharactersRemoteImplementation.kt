...
class CharactersRemoteImplementation constructor(
    ...
    /**
     * Retrieve a list of [Character] from the [MarvelHeroesService]
     */
    override fun getCharacters(timestamp: String, hash: String): Single<List<Character>>? {
        return marvelHeroesService.getCharacters(timestamp, BuildConfig.API_KEY, hash)
            .map { data ->
    ...
    /**
     * Retrieve one [Character] from the [MarvelHeroesService]
     */
    override fun getCharacterDetail(timestamp: String, hash: String, characterId: Int?): Single<Character>? {
        return marvelHeroesService.getCharacterDetail(characterId.toString(), timestamp, BuildConfig.API_KEY, hash)
            .map { data ->
